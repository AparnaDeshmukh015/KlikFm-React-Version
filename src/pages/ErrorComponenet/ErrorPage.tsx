...
const ErrorPage = (props: any) => {
  ...
  useEffect(() => {
    ...
    if (code === "404") {
      ...
      setStatusPara(
        " Sorry, The Request method is not implemented by the server!"
      );
    } else if (code === "502") {
      setStatusCode(code);
  ...
  return (
    <section className="w-full">
      <div className="mt-8">
        <center>
          <h1 className="mb-2">{statusCode}</h1>
          <h4>
            {statusMessage}{" "}
            <i
              className="pi pi-exclamation-triangle text-amber-500"
              style={{ fontSize: "2.5rem" }}
            ></i>
          </h4>
          <div className="error-details mb-7 Text_Secondary ">{statusPara}</div>
          <a
            href={`${process.env.REACT_APP_REDIRECT_URL}workorderlist`}
