...
const AssetHierarchyMasterForm = (props: any) => {
  ...
  const onSubmit = useCallback(
    async (payload: any) => {
      ...
      payload.ASSET_FOLDER_DESCRIPTION =
        payload?.PARENT_ASSET_FOLDER_ID?.ASSET_FOLDER_DESCRIPTION;
      payload.PARENT_ASSET_FOLDER_ID =
        payload?.PARENT_ASSET_FOLDER_ID?.ASSET_FOLDER_ID ?? 0;
  ...
  const getOptions = async () => {
    try {
      const res = await callPostAPI(ENDPOINTS.GET_EQUIPMENT_HIERARCHY);

      setOptions({
        assetHierarchyList: res?.EQUIPMENTHIERARCHYLIST?.filter(
          (item: any) => item?.ASSET_FOLDER_DESCRIPTION !== null
        ),
      });
    } catch (error) {}
  };
